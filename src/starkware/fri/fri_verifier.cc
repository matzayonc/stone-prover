...
namespace starkware
{
  ...
  void FriVerifier::VerifyInnerLayers()
  {
    ...
    for (size_t i = 0; i < n_layers_ - 1; ++i)
    {
      ...
      std::cerr << "VerifyInnerLayers" << i << " of " << n_layers_ << std::endl;
      std::cerr << "basis_index" << basis_index << std::endl;
      std::cerr << "first_fri_step" << first_fri_step << std::endl;
      std::cerr << "first_fri_step" << first_fri_step << std::endl;
      ...
      std::cerr << "query_indices_.size()" << query_indices_.size() << std::endl;
      std::cerr << "to_verify.size()" << to_verify.size() << std::endl;
      std::cerr << "query_results_.size()" << query_results_.size() << std::endl;
      ...
      for (size_t j = 0; j < query_results_.size(); ++j)
      {
        ...
        std::cerr << "coset_size" << coset_size << std::endl;
