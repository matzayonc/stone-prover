...
namespace starkware
{
    ...
    */
    ...
    std::vector<uint64_t> GetWidths(const std::vector<TracePtrClass> &trace_ptrs)
    {
      ...
      for (const auto &trace_ptr : trace_ptrs)
      {
        ...
        std::cerr << "trace_ptr->NumColumns(): " << trace_ptr->NumColumns() << std::endl;
      }
      ...
      const FieldElement &point, const FieldElementSpan &output) const
  {
    ...
    for (size_t mask_i = 0; mask_i < mask_.size(); ++mask_i)
    {
      ...
      std::cerr << "Traces log size: " << traces_.size() << std::endl;
      std::cerr << "Split mask " << trace_i << ": " << split_masks_[trace_i].size() << std::endl;
      ...
      std::vector<std::pair<uint64_t, uint64_t>> queries) const
  {
    ...
    std::cerr << "traces_.size(): " << traces_.size() << std::endl;
    std::cerr << "split_masks_[2].size(): " << split_masks_[2].size() << std::endl;
    ...
    std::cerr << "trace_mask_values.size(): " << trace_mask_values.size() << std::endl;
    std::cerr << "mask_.size(): " << mask_.size() << std::endl;

    // Compute composition polynomial at queries.
    const auto sizes = GetWidths(traces_);
    const size_t log_n_cosets = SafeLog2(evaluation_domain_->NumCosets());
    std::cerr << "log_n_cosets: " << log_n_cosets << std::endl;
