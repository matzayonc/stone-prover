...
namespace starkware
{
      ...
      const HashT &merkle_root, VerifierChannel *channel)
  {
    ...
    std::cerr << "VerifyDecommitment queue.size(): " << queue.size() << std::endl;
    std::cerr << "VerifyDecommitment total_data_length: " << total_data_length << std::endl;
    std::cerr << "VerifyDecommitment data_to_verify: " << data_to_verify.size() << std::endl;
